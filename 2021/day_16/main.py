#!/usr/bin/env python
from math import prod

SUM, PRODUCT, MIN, MAX, LITERAL, GT, LT, EQ = range(8)

OPS = {
    SUM: sum,
    PRODUCT: prod,
    MIN: min,
    MAX: max,
    GT: lambda subs: subs[0] > subs[1],
    LT: lambda subs: subs[0] < subs[1],
    EQ: lambda subs: subs[0] == subs[1],
}


def decode(pkg):
    versions = int(pkg[:3], 2)
    type_id = int(pkg[3:6], 2)

    if type_id == LITERAL:
        num = 0
        todo = pkg[6:]
        while True:
            num <<= 4
            num |= int(todo[1:5], 2)
            if todo[0] == "0":
                return versions, num, todo[5:]
            todo = todo[5:]

    subs = []

    if pkg[6] == "0":
        length = int(pkg[7:22], 2)
        todo, pkg = pkg[22 : 22 + length], pkg[22 + length :]
        while todo:
            sub_version, sub, todo = decode(todo)
            subs.append(sub)
            versions += sub_version
    else:
        count = int(pkg[7:18], 2)
        pkg = pkg[18:]
        for _ in range(count):
            sub_version, sub, pkg = decode(pkg)
            subs.append(sub)
            versions += sub_version

    return versions, OPS[type_id](subs), pkg


data = "".join(f"{int(c, 16):04b}" for c in open("input").read().strip())

version, result, _ = decode(data)

print(version)
print(result)
